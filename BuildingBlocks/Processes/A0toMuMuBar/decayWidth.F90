...
#include "looptools.h"
    ...
    double precision A0toMuMuBarTree, A0toMuMuBarCT, A0toMuMuBarReal, treeLevelTemp, realCorrectionsTemp
    double complex A0toMuMuBarVC, A0toMuMuBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    UVChecks(1) = DBLE(A0toMuMuBarVC()) + A0toMuMuBarCT(n)
                else
                    UVChecks(1) = DBLE(A0toMuMuBarVC() + A0toMuMuBarTad()) + &
                                & A0toMuMuBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        UVChecks(m) = ( DBLE(A0toMuMuBarVC()) + A0toMuMuBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(A0toMuMuBarVC() + A0toMuMuBarTad()) + &
                                    & A0toMuMuBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
    ...
    if (arguments(2) == 1) then
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MA0**4 + MM**4 + MM**4 - 2D0*MA0**2*MM**2 - 2D0*MA0**2*MM**2 &
                        &  - 2D0*MM**2*MM**2 )/(16D0*PI*MA0**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(A0toMuMuBarVC()) + &
                                & 2D0*A0toMuMuBarCT(n) + &
                                & A0toMuMuBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(A0toMuMuBarVC() + &
                                & A0toMuMuBarTad()) + 2D0*A0toMuMuBarCT(n) + &
                                & A0toMuMuBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(A0toMuMuBarVC()) + &
                                    & 2D0*A0toMuMuBarCT(n) + A0toMuMuBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(A0toMuMuBarVC() + &
                                    & A0toMuMuBarTad()) + 2D0*A0toMuMuBarCT(n) + &
                                    & A0toMuMuBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", A0toMuMuBarReal()
    ...
    if (arguments(3) >= 1) then
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    call clearcache
                    GaugeXiAChecks(5) = A0toMuMuBarTree() + &
                        & 2D0*DBLE(A0toMuMuBarVC()) + A0toMuMuBarReal() + &
                        & 2D0*A0toMuMuBarCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = A0toMuMuBarTree() + &
                        & 2D0*DBLE(A0toMuMuBarVC()) + A0toMuMuBarReal() + &
                        & 2D0*A0toMuMuBarCT(n)
                    call clearcache
                    GaugeXiZChecks(5) = A0toMuMuBarTree() + &
                        & 2D0*DBLE(A0toMuMuBarVC()) + A0toMuMuBarReal() + &
                        & 2D0*A0toMuMuBarCT(n)
                else
                    call clearcache
                    GaugeXiAChecks(5) = A0toMuMuBarTree() + &
                        & 2D0*DBLE(A0toMuMuBarVC() + A0toMuMuBarTad()) + &
                        & A0toMuMuBarReal() + 2D0*A0toMuMuBarCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = A0toMuMuBarTree() + &
                        & 2D0*DBLE(A0toMuMuBarVC() + A0toMuMuBarTad()) + &
                        & A0toMuMuBarReal() + 2D0*A0toMuMuBarCT(n)
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiAChecks(m) = ( A0toMuMuBarTree() + &
                            & 2D0*DBLE(A0toMuMuBarVC()) + 2D0*A0toMuMuBarCT(n) + &
                            & A0toMuMuBarReal() - GaugeXiAChecks(5) )/GaugeXiAChecks(5)
                    else
                        GaugeXiAChecks(m) = ( A0toMuMuBarTree() + &
                            & 2D0*DBLE(A0toMuMuBarVC() + A0toMuMuBarTad()) + &
                            & 2D0*A0toMuMuBarCT(n) + A0toMuMuBarReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiWChecks(m) = ( A0toMuMuBarTree() + &
                            & 2D0*DBLE(A0toMuMuBarVC()) + 2D0*A0toMuMuBarCT(n) + &
                            & A0toMuMuBarReal() - GaugeXiWChecks(5) )/GaugeXiWChecks(5)
                    else
                        GaugeXiWChecks(m) = ( A0toMuMuBarTree() + &
                            & 2D0*DBLE(A0toMuMuBarVC() + A0toMuMuBarTad()) + &
                            & 2D0*A0toMuMuBarCT(n) + A0toMuMuBarReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiZChecks(m) = ( A0toMuMuBarTree() + &
                            & 2D0*DBLE(A0toMuMuBarVC()) + 2D0*A0toMuMuBarCT(n) + &
                            & A0toMuMuBarReal() - GaugeXiZChecks(5) )/GaugeXiZChecks(5)
                    else
                        GaugeXiZChecks(m) = ( A0toMuMuBarTree() + &
                            & 2D0*DBLE(A0toMuMuBarVC() + A0toMuMuBarTad()) + &
                            & 2D0*A0toMuMuBarCT(n) + A0toMuMuBarReal() - &
    ...
    if (arguments(4) == 1) then
        ...
        call ltini
            ...
            do point = 1, maxPoint, 1
                ...
                call clearcache
                treeLevelWidth = prefactor*A0toMuMuBarTree()
