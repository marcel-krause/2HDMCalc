double precision function A0toZ0h0Real()
 use constants
 use counterterms
 implicit none
#include "looptools.h"
...
!    & (Qssv0 + Qssv1)**2*Ossv33
 ...
 totalAmplitude = 1D0*1D0*(EL2/(2D0*(4D0*PI)**3*m1))*&
     &(16D0*PI*m1**3)/DSQRT(m1**4 + m2**4 + m3**4 - 2D0*m1**2*m2**2 - 2D0*m1**2*m3**2 - 2D0*m2**2*m3**2 )*&
     &( 1D0*( OLLFull ) + 1D0*( OLRFull ) )

 A0toZ0h0Real = totalAmplitude
end function A0toZ0h0Real
