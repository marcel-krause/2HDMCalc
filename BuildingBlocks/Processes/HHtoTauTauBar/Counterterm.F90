double precision function HHtoTauTauBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSUsual()/ML - dMW2Usual()/(2D0*MW2) + Yuk6*dBeta1KanUsual() + &
			& Yuk4/Yuk5*dAlphaKanUsual() + dZHHHHOS()/2D0 + Yuk4/Yuk5 * dZh0HHOSUsual()/2D0 + dZTauTauOSLeft()/2D0 + &
	...
	case (3)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk6*dBeta1KanAlter() + &
			& Yuk4/Yuk5*dAlphaKanAlter() + dZHHHHOS()/2D0 + Yuk4/Yuk5 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk5**INT(2.D0)))/(MW2*SW2)
	case (4)
	...
	case (5)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk6*dBeta1PinchPStar() + &
			& Yuk4/Yuk5*dAlphaPinchPStar() + dZHHHHOS()/2D0 + Yuk4/Yuk5 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk5**INT(2.D0)))/(MW2*SW2)
	case (6)
 ...
 HHtoTauTauBarCT = totalAmplitude
end function HHtoTauTauBarCT
