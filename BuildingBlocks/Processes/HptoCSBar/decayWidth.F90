...
#include "looptools.h"
    ...
    double precision HptoCSBarTree, HptoCSBarCT, HptoCSBarReal, treeLevelTemp, realCorrectionsTemp
    double complex HptoCSBarVC, HptoCSBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    UVChecks(1) = DBLE(HptoCSBarVC()) + HptoCSBarCT(n)
                else
                    UVChecks(1) = DBLE(HptoCSBarVC() + HptoCSBarTad()) + &
                                & HptoCSBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        UVChecks(m) = ( DBLE(HptoCSBarVC()) + HptoCSBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(HptoCSBarVC() + HptoCSBarTad()) + &
                                    & HptoCSBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
    ...
    if (arguments(2) == 1) then
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MHp**4 + MC**4 + MS**4 - 2D0*MHp**2*MC**2 - 2D0*MHp**2*MS**2 &
                        &  - 2D0*MC**2*MS**2 )/(16D0*PI*MHp**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoCSBarVC()) + &
                                & 2D0*HptoCSBarCT(n) + &
                                & HptoCSBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoCSBarVC() + &
                                & HptoCSBarTad()) + 2D0*HptoCSBarCT(n) + &
                                & HptoCSBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoCSBarVC()) + &
                                    & 2D0*HptoCSBarCT(n) + HptoCSBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoCSBarVC() + &
                                    & HptoCSBarTad()) + 2D0*HptoCSBarCT(n) + &
                                    & HptoCSBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", HptoCSBarReal()
    ...
    if (arguments(3) >= 1) then
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    call clearcache
                    GaugeXiAChecks(5) = HptoCSBarTree() + &
                        & 2D0*DBLE(HptoCSBarVC()) + HptoCSBarReal() + &
                        & 2D0*HptoCSBarCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = HptoCSBarTree() + &
                        & 2D0*DBLE(HptoCSBarVC()) + HptoCSBarReal() + &
                        & 2D0*HptoCSBarCT(n)
                    call clearcache
                    GaugeXiZChecks(5) = HptoCSBarTree() + &
                        & 2D0*DBLE(HptoCSBarVC()) + HptoCSBarReal() + &
                        & 2D0*HptoCSBarCT(n)
                else
                    call clearcache
                    GaugeXiAChecks(5) = HptoCSBarTree() + &
                        & 2D0*DBLE(HptoCSBarVC() + HptoCSBarTad()) + &
                        & HptoCSBarReal() + 2D0*HptoCSBarCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = HptoCSBarTree() + &
                        & 2D0*DBLE(HptoCSBarVC() + HptoCSBarTad()) + &
                        & HptoCSBarReal() + 2D0*HptoCSBarCT(n)
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiAChecks(m) = ( HptoCSBarTree() + &
                            & 2D0*DBLE(HptoCSBarVC()) + 2D0*HptoCSBarCT(n) + &
                            & HptoCSBarReal() - GaugeXiAChecks(5) )/GaugeXiAChecks(5)
                    else
                        GaugeXiAChecks(m) = ( HptoCSBarTree() + &
                            & 2D0*DBLE(HptoCSBarVC() + HptoCSBarTad()) + &
                            & 2D0*HptoCSBarCT(n) + HptoCSBarReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiWChecks(m) = ( HptoCSBarTree() + &
                            & 2D0*DBLE(HptoCSBarVC()) + 2D0*HptoCSBarCT(n) + &
                            & HptoCSBarReal() - GaugeXiWChecks(5) )/GaugeXiWChecks(5)
                    else
                        GaugeXiWChecks(m) = ( HptoCSBarTree() + &
                            & 2D0*DBLE(HptoCSBarVC() + HptoCSBarTad()) + &
                            & 2D0*HptoCSBarCT(n) + HptoCSBarReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiZChecks(m) = ( HptoCSBarTree() + &
                            & 2D0*DBLE(HptoCSBarVC()) + 2D0*HptoCSBarCT(n) + &
                            & HptoCSBarReal() - GaugeXiZChecks(5) )/GaugeXiZChecks(5)
                    else
                        GaugeXiZChecks(m) = ( HptoCSBarTree() + &
                            & 2D0*DBLE(HptoCSBarVC() + HptoCSBarTad()) + &
                            & 2D0*HptoCSBarCT(n) + HptoCSBarReal() - &
    ...
    if (arguments(4) == 1) then
        ...
        call ltini
            ...
            do point = 1, maxPoint, 1
                ...
                call clearcache
                treeLevelWidth = prefactor*HptoCSBarTree()
