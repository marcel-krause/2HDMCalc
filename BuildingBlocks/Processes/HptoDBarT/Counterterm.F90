...
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		dcRight = EL*CKM32/(DSQRT(2D0)*MW*SW)*MD*Yuk3*( &
		...
		totalAmplitude = 3D0*(MHp2 - (MT + MD)**2)*(MT/TB + MD*Yuk3)*EL*CKM31/(DSQRT(2D0)*MW*SW)*(dcLeft + dcRight)/2D0 + &
					   & 3D0*(MHp2 - (MT - MD)**2)*(MT/TB - MD*Yuk3)*EL*CKM31/(DSQRT(2D0)*MW*SW)*(dcLeft - dcRight)/2D0
	case (2)
		dcLeft = EL*CKM31/(DSQRT(2D0)*MW*SW)*MT/TB*( &
			& dZHpHpOS()/2D0 + TB*dZGpHpOSUsual()/2D0 + &
			& (CKM11/CKM31*MU/MT*dZUTOSRight() + CKM31/CKM31*dZDDOSLeft())/2D0 + &
