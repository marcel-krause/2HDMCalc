...
#include "looptools.h"
    ...
    double precision HptoDBarTTree, HptoDBarTCT, HptoDBarTReal, treeLevelTemp, realCorrectionsTemp
    double complex HptoDBarTVC, HptoDBarTTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    UVChecks(1) = DBLE(HptoDBarTVC()) + HptoDBarTCT(n)
                else
                    UVChecks(1) = DBLE(HptoDBarTVC() + HptoDBarTTad()) + &
                                & HptoDBarTCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        UVChecks(m) = ( DBLE(HptoDBarTVC()) + HptoDBarTCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(HptoDBarTVC() + HptoDBarTTad()) + &
                                    & HptoDBarTCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
    ...
    if (arguments(2) == 1) then
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MHp**4 + MD**4 + MT**4 - 2D0*MHp**2*MD**2 - 2D0*MHp**2*MT**2 &
                        &  - 2D0*MD**2*MT**2 )/(16D0*PI*MHp**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoDBarTVC()) + &
                                & 2D0*HptoDBarTCT(n) + &
                                & HptoDBarTReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoDBarTVC() + &
                                & HptoDBarTTad()) + 2D0*HptoDBarTCT(n) + &
                                & HptoDBarTReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoDBarTVC()) + &
                                    & 2D0*HptoDBarTCT(n) + HptoDBarTReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoDBarTVC() + &
                                    & HptoDBarTTad()) + 2D0*HptoDBarTCT(n) + &
                                    & HptoDBarTReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", HptoDBarTReal()
    ...
    if (arguments(3) >= 1) then
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    call clearcache
                    GaugeXiAChecks(5) = HptoDBarTTree() + &
                        & 2D0*DBLE(HptoDBarTVC()) + HptoDBarTReal() + &
                        & 2D0*HptoDBarTCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = HptoDBarTTree() + &
                        & 2D0*DBLE(HptoDBarTVC()) + HptoDBarTReal() + &
                        & 2D0*HptoDBarTCT(n)
                    call clearcache
                    GaugeXiZChecks(5) = HptoDBarTTree() + &
                        & 2D0*DBLE(HptoDBarTVC()) + HptoDBarTReal() + &
                        & 2D0*HptoDBarTCT(n)
                else
                    call clearcache
                    GaugeXiAChecks(5) = HptoDBarTTree() + &
                        & 2D0*DBLE(HptoDBarTVC() + HptoDBarTTad()) + &
                        & HptoDBarTReal() + 2D0*HptoDBarTCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = HptoDBarTTree() + &
                        & 2D0*DBLE(HptoDBarTVC() + HptoDBarTTad()) + &
                        & HptoDBarTReal() + 2D0*HptoDBarTCT(n)
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiAChecks(m) = ( HptoDBarTTree() + &
                            & 2D0*DBLE(HptoDBarTVC()) + 2D0*HptoDBarTCT(n) + &
                            & HptoDBarTReal() - GaugeXiAChecks(5) )/GaugeXiAChecks(5)
                    else
                        GaugeXiAChecks(m) = ( HptoDBarTTree() + &
                            & 2D0*DBLE(HptoDBarTVC() + HptoDBarTTad()) + &
                            & 2D0*HptoDBarTCT(n) + HptoDBarTReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiWChecks(m) = ( HptoDBarTTree() + &
                            & 2D0*DBLE(HptoDBarTVC()) + 2D0*HptoDBarTCT(n) + &
                            & HptoDBarTReal() - GaugeXiWChecks(5) )/GaugeXiWChecks(5)
                    else
                        GaugeXiWChecks(m) = ( HptoDBarTTree() + &
                            & 2D0*DBLE(HptoDBarTVC() + HptoDBarTTad()) + &
                            & 2D0*HptoDBarTCT(n) + HptoDBarTReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiZChecks(m) = ( HptoDBarTTree() + &
                            & 2D0*DBLE(HptoDBarTVC()) + 2D0*HptoDBarTCT(n) + &
                            & HptoDBarTReal() - GaugeXiZChecks(5) )/GaugeXiZChecks(5)
                    else
                        GaugeXiZChecks(m) = ( HptoDBarTTree() + &
                            & 2D0*DBLE(HptoDBarTVC() + HptoDBarTTad()) + &
                            & 2D0*HptoDBarTCT(n) + HptoDBarTReal() - &
    ...
    if (arguments(4) == 1) then
        ...
        call ltini
            ...
            do point = 1, maxPoint, 1
                ...
                call clearcache
                treeLevelWidth = prefactor*HptoDBarTTree()
