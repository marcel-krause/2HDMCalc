double precision function HptoSBarUReal()
 use constants
 use counterterms
 implicit none
#include "looptools.h"
...
!    & (Qssv0 + Qssv1)**2*Ossv33
 ...
 totalAmplitude = 3D0*EL2*CKM12**2/(2D0*MW2*SW2)*(EL2/(2D0*(4D0*PI)**3*m1))*&
                &(16D0*PI*m1**3)/DSQRT(m1**4 + m2**4 + m3**4 - 2D0*m1**2*m2**2 - 2D0*m1**2*m3**2 - 2D0*m2**2*m3**2 )*&
                &( (m3**2/TB2 + m2**2*Yuk3**2)*( OLLFull ) + 2D0*m2*m3*Yuk3/TB*( OLRFull ) )

 HptoSBarUReal = totalAmplitude
end function HptoSBarUReal
