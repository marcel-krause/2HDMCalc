...
#include "looptools.h"
    ...
    double precision h0toA0A0Tree, h0toA0A0CT, h0toA0A0Real, treeLevelTemp, realCorrectionsTemp
    double complex h0toA0A0VC, h0toA0A0Tad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    UVChecks(1) = DBLE(h0toA0A0VC()) + h0toA0A0CT(n)
                else
                    UVChecks(1) = DBLE(h0toA0A0VC() + h0toA0A0Tad()) + &
                                & h0toA0A0CT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        UVChecks(m) = ( DBLE(h0toA0A0VC()) + h0toA0A0CT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(h0toA0A0VC() + h0toA0A0Tad()) + &
                                    & h0toA0A0CT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
    ...
    if (arguments(2) == 1) then
        ...
        call ltini
            ...
            prefactor = 1D0/2D0 * DSQRT(Mh0**4 + MA0**4 + MA0**4 - 2D0*Mh0**2*MA0**2 - 2D0*Mh0**2*MA0**2 &
                        &  - 2D0*MA0**2*MA0**2 )/(16D0*PI*Mh0**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(h0toA0A0VC()) + &
                                & 2D0*h0toA0A0CT(n) + &
                                & h0toA0A0Real() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(h0toA0A0VC() + &
                                & h0toA0A0Tad()) + 2D0*h0toA0A0CT(n) + &
                                & h0toA0A0Real() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(h0toA0A0VC()) + &
                                    & 2D0*h0toA0A0CT(n) + h0toA0A0Real() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(h0toA0A0VC() + &
                                    & h0toA0A0Tad()) + 2D0*h0toA0A0CT(n) + &
                                    & h0toA0A0Real() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", h0toA0A0Real()
    ...
    if (arguments(3) >= 1) then
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    call clearcache
                    GaugeXiAChecks(5) = h0toA0A0Tree() + &
                        & 2D0*DBLE(h0toA0A0VC()) + h0toA0A0Real() + &
                        & 2D0*h0toA0A0CT(n)
                    call clearcache
                    GaugeXiWChecks(5) = h0toA0A0Tree() + &
                        & 2D0*DBLE(h0toA0A0VC()) + h0toA0A0Real() + &
                        & 2D0*h0toA0A0CT(n)
                    call clearcache
                    GaugeXiZChecks(5) = h0toA0A0Tree() + &
                        & 2D0*DBLE(h0toA0A0VC()) + h0toA0A0Real() + &
                        & 2D0*h0toA0A0CT(n)
                else
                    call clearcache
                    GaugeXiAChecks(5) = h0toA0A0Tree() + &
                        & 2D0*DBLE(h0toA0A0VC() + h0toA0A0Tad()) + &
                        & h0toA0A0Real() + 2D0*h0toA0A0CT(n)
                    call clearcache
                    GaugeXiWChecks(5) = h0toA0A0Tree() + &
                        & 2D0*DBLE(h0toA0A0VC() + h0toA0A0Tad()) + &
                        & h0toA0A0Real() + 2D0*h0toA0A0CT(n)
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiAChecks(m) = ( h0toA0A0Tree() + &
                            & 2D0*DBLE(h0toA0A0VC()) + 2D0*h0toA0A0CT(n) + &
                            & h0toA0A0Real() - GaugeXiAChecks(5) )/GaugeXiAChecks(5)
                    else
                        GaugeXiAChecks(m) = ( h0toA0A0Tree() + &
                            & 2D0*DBLE(h0toA0A0VC() + h0toA0A0Tad()) + &
                            & 2D0*h0toA0A0CT(n) + h0toA0A0Real() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiWChecks(m) = ( h0toA0A0Tree() + &
                            & 2D0*DBLE(h0toA0A0VC()) + 2D0*h0toA0A0CT(n) + &
                            & h0toA0A0Real() - GaugeXiWChecks(5) )/GaugeXiWChecks(5)
                    else
                        GaugeXiWChecks(m) = ( h0toA0A0Tree() + &
                            & 2D0*DBLE(h0toA0A0VC() + h0toA0A0Tad()) + &
                            & 2D0*h0toA0A0CT(n) + h0toA0A0Real() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiZChecks(m) = ( h0toA0A0Tree() + &
                            & 2D0*DBLE(h0toA0A0VC()) + 2D0*h0toA0A0CT(n) + &
                            & h0toA0A0Real() - GaugeXiZChecks(5) )/GaugeXiZChecks(5)
                    else
                        GaugeXiZChecks(m) = ( h0toA0A0Tree() + &
                            & 2D0*DBLE(h0toA0A0VC() + h0toA0A0Tad()) + &
                            & 2D0*h0toA0A0CT(n) + h0toA0A0Real() - &
    ...
    if (arguments(4) == 1) then
        ...
        call ltini
            ...
            do point = 1, maxPoint, 1
                ...
                call clearcache
                treeLevelWidth = prefactor*h0toA0A0Tree()
