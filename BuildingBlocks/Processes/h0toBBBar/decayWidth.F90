...
#include "looptools.h"
    ...
    double precision h0toBBBarTree, h0toBBBarCT, h0toBBBarReal, treeLevelTemp, realCorrectionsTemp
    double complex h0toBBBarVC, h0toBBBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
    ...
    if (arguments(1) == 1) then
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2)) then
                    UVChecks(1) = DBLE(h0toBBBarVC()) + h0toBBBarCT(n)
                else
                    UVChecks(1) = DBLE(h0toBBBarVC() + h0toBBBarTad()) + &
                                & h0toBBBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1 and 2 are without tadpoles
                    if ((n == 1) .OR. (n == 2)) then
                        UVChecks(m) = ( DBLE(h0toBBBarVC()) + h0toBBBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(h0toBBBarVC() + h0toBBBarTad()) + &
                                    & h0toBBBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
    ...
    if (arguments(2) == 1) then
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(Mh0**4 + MB**4 + MB**4 - 2D0*Mh0**2*MB**2 - 2D0*Mh0**2*MB**2 &
                        &  - 2D0*MB**2*MB**2 )/(16D0*PI*Mh0**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1 and 2 are without tadpoles
                if ((n == 1) .OR. (n == 2)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(h0toBBBarVC()) + &
                                & 2D0*h0toBBBarCT(n) + &
                                & h0toBBBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(h0toBBBarVC() + &
                                & h0toBBBarTad()) + 2D0*h0toBBBarCT(n) + &
                                & h0toBBBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1 and 2 are without tadpoles
                    if ((n == 1) .OR. (n == 2)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(h0toBBBarVC()) + &
                                    & 2D0*h0toBBBarCT(n) + h0toBBBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(h0toBBBarVC() + &
                                    & h0toBBBarTad()) + 2D0*h0toBBBarCT(n) + &
                                    & h0toBBBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", h0toBBBarReal()
    ...
    if (arguments(3) >= 1) then
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2)) then
                    call clearcache
                    GaugeXiAChecks(5) = h0toBBBarTree() + &
                        & 2D0*DBLE(h0toBBBarVC()) + h0toBBBarReal() + &
                        & 2D0*h0toBBBarCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = h0toBBBarTree() + &
                        & 2D0*DBLE(h0toBBBarVC()) + h0toBBBarReal() + &
                        & 2D0*h0toBBBarCT(n)
                    call clearcache
                    GaugeXiZChecks(5) = h0toBBBarTree() + &
                        & 2D0*DBLE(h0toBBBarVC()) + h0toBBBarReal() + &
                        & 2D0*h0toBBBarCT(n)
                else
                    call clearcache
                    GaugeXiAChecks(5) = h0toBBBarTree() + &
                        & 2D0*DBLE(h0toBBBarVC() + h0toBBBarTad()) + &
                        & h0toBBBarReal() + 2D0*h0toBBBarCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = h0toBBBarTree() + &
                        & 2D0*DBLE(h0toBBBarVC() + h0toBBBarTad()) + &
                        & h0toBBBarReal() + 2D0*h0toBBBarCT(n)
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1 and 2 are without tadpoles
                    if ((n == 1) .OR. (n == 2)) then
                        GaugeXiAChecks(m) = ( h0toBBBarTree() + &
                            & 2D0*DBLE(h0toBBBarVC()) + 2D0*h0toBBBarCT(n) + &
                            & h0toBBBarReal() - GaugeXiAChecks(5) )/GaugeXiAChecks(5)
                    else
                        GaugeXiAChecks(m) = ( h0toBBBarTree() + &
                            & 2D0*DBLE(h0toBBBarVC() + h0toBBBarTad()) + &
                            & 2D0*h0toBBBarCT(n) + h0toBBBarReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1 and 2 are without tadpoles
                    if ((n == 1) .OR. (n == 2)) then
                        GaugeXiWChecks(m) = ( h0toBBBarTree() + &
                            & 2D0*DBLE(h0toBBBarVC()) + 2D0*h0toBBBarCT(n) + &
                            & h0toBBBarReal() - GaugeXiWChecks(5) )/GaugeXiWChecks(5)
                    else
                        GaugeXiWChecks(m) = ( h0toBBBarTree() + &
                            & 2D0*DBLE(h0toBBBarVC() + h0toBBBarTad()) + &
                            & 2D0*h0toBBBarCT(n) + h0toBBBarReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1 and 2 are without tadpoles
                    if ((n == 1) .OR. (n == 2)) then
                        GaugeXiZChecks(m) = ( h0toBBBarTree() + &
                            & 2D0*DBLE(h0toBBBarVC()) + 2D0*h0toBBBarCT(n) + &
                            & h0toBBBarReal() - GaugeXiZChecks(5) )/GaugeXiZChecks(5)
                    else
                        GaugeXiZChecks(m) = ( h0toBBBarTree() + &
                            & 2D0*DBLE(h0toBBBarVC() + h0toBBBarTad()) + &
                            & 2D0*h0toBBBarCT(n) + h0toBBBarReal() - &
    ...
    if (arguments(4) == 1) then
        ...
        call ltini
            ...
            do point = 1, maxPoint, 1
                ...
                call clearcache
                treeLevelWidth = prefactor*h0toBBBarTree()
