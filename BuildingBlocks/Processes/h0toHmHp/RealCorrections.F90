double precision function h0toHmHpReal()
 use constants
 use counterterms
 implicit none
#include "looptools.h"
...
OsssFull = Qsss1*Qsss1*Osss11 + Qsss1*Qsss2*Osss12 + Qsss2*Qsss2*Osss22
 ...
 totalAmplitude = 1D0*((0.25D0*EL2*DBLE(((Mh02 - 2.D0*MHp2)*SBA - (1.D0*CAB*(2.D0*Mh02 - (4.D0*Lambda5*MW2*SW2)&
  &/EL2))/S2B)**INT(2.D0)))/(MW2*SW2))*(EL2/(2D0*(4D0*PI)**3*m1))*&
     &(16D0*PI*m1**3)/DSQRT(m1**4 + m2**4 + m3**4 - 2D0*m1**2*m2**2 - 2D0*m1**2*m3**2 - 2D0*m2**2*m3**2 )*&
     &( OsssFull )

 h0toHmHpReal = totalAmplitude
end function h0toHmHpReal
