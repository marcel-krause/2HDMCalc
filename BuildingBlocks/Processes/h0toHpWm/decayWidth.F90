...
#include "looptools.h"
    ...
    double precision h0toHpWmTree, h0toHpWmCT, h0toHpWmReal, treeLevelTemp, realCorrectionsTemp
    double complex h0toHpWmVC, h0toHpWmTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    UVChecks(1) = DBLE(h0toHpWmVC()) + h0toHpWmCT(n)
                else
                    UVChecks(1) = DBLE(h0toHpWmVC() + h0toHpWmTad()) + &
                                & h0toHpWmCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        UVChecks(m) = ( DBLE(h0toHpWmVC()) + h0toHpWmCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(h0toHpWmVC() + h0toHpWmTad()) + &
                                    & h0toHpWmCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
    ...
    if (arguments(2) == 1) then
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(Mh0**4 + MHp**4 + MW**4 - 2D0*Mh0**2*MHp**2 - 2D0*Mh0**2*MW**2 &
                        &  - 2D0*MHp**2*MW**2 )/(16D0*PI*Mh0**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(h0toHpWmVC()) + &
                                & 2D0*h0toHpWmCT(n) + &
                                & h0toHpWmReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(h0toHpWmVC() + &
                                & h0toHpWmTad()) + 2D0*h0toHpWmCT(n) + &
                                & h0toHpWmReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(h0toHpWmVC()) + &
                                    & 2D0*h0toHpWmCT(n) + h0toHpWmReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(h0toHpWmVC() + &
                                    & h0toHpWmTad()) + 2D0*h0toHpWmCT(n) + &
                                    & h0toHpWmReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", h0toHpWmReal()
    ...
    if (arguments(3) >= 1) then
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                    call clearcache
                    GaugeXiAChecks(5) = h0toHpWmTree() + &
                        & 2D0*DBLE(h0toHpWmVC()) + h0toHpWmReal() + &
                        & 2D0*h0toHpWmCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = h0toHpWmTree() + &
                        & 2D0*DBLE(h0toHpWmVC()) + h0toHpWmReal() + &
                        & 2D0*h0toHpWmCT(n)
                    call clearcache
                    GaugeXiZChecks(5) = h0toHpWmTree() + &
                        & 2D0*DBLE(h0toHpWmVC()) + h0toHpWmReal() + &
                        & 2D0*h0toHpWmCT(n)
                else
                    call clearcache
                    GaugeXiAChecks(5) = h0toHpWmTree() + &
                        & 2D0*DBLE(h0toHpWmVC() + h0toHpWmTad()) + &
                        & h0toHpWmReal() + 2D0*h0toHpWmCT(n)
                    call clearcache
                    GaugeXiWChecks(5) = h0toHpWmTree() + &
                        & 2D0*DBLE(h0toHpWmVC() + h0toHpWmTad()) + &
                        & h0toHpWmReal() + 2D0*h0toHpWmCT(n)
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiAChecks(m) = ( h0toHpWmTree() + &
                            & 2D0*DBLE(h0toHpWmVC()) + 2D0*h0toHpWmCT(n) + &
                            & h0toHpWmReal() - GaugeXiAChecks(5) )/GaugeXiAChecks(5)
                    else
                        GaugeXiAChecks(m) = ( h0toHpWmTree() + &
                            & 2D0*DBLE(h0toHpWmVC() + h0toHpWmTad()) + &
                            & 2D0*h0toHpWmCT(n) + h0toHpWmReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiWChecks(m) = ( h0toHpWmTree() + &
                            & 2D0*DBLE(h0toHpWmVC()) + 2D0*h0toHpWmCT(n) + &
                            & h0toHpWmReal() - GaugeXiWChecks(5) )/GaugeXiWChecks(5)
                    else
                        GaugeXiWChecks(m) = ( h0toHpWmTree() + &
                            & 2D0*DBLE(h0toHpWmVC() + h0toHpWmTad()) + &
                            & 2D0*h0toHpWmCT(n) + h0toHpWmReal() - &
                ...
                do m = 1, 10, 1
                    ...
                    ! Schemes 1, 2, 9, 11 and 13 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9) .OR. (n == 11) .OR. (n == 13)) then
                        GaugeXiZChecks(m) = ( h0toHpWmTree() + &
                            & 2D0*DBLE(h0toHpWmVC()) + 2D0*h0toHpWmCT(n) + &
                            & h0toHpWmReal() - GaugeXiZChecks(5) )/GaugeXiZChecks(5)
                    else
                        GaugeXiZChecks(m) = ( h0toHpWmTree() + &
                            & 2D0*DBLE(h0toHpWmVC() + h0toHpWmTad()) + &
                            & 2D0*h0toHpWmCT(n) + h0toHpWmReal() - &
    ...
    if (arguments(4) == 1) then
        ...
        call ltini
            ...
            do point = 1, maxPoint, 1
                ...
                call clearcache
                treeLevelWidth = prefactor*h0toHpWmTree()
