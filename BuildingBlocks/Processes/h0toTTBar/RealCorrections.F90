double precision function h0toTTBarReal()
 use constants
 use counterterms
 implicit none
#include "looptools.h"
...
! Osss22 = -4D0*Id3Fin - 8D0*I33*m3**2
 ...
 OLLFull = 4D0/9D0*( OLL11 + OLL12 + OLL22 )
 OLRFull = 4D0/9D0*( OLR11 + OLR12 + OLR22 )
...
! OsssFull = Qsss1*Qsss1*Osss11 + Qsss1*Qsss2*Osss12 + Qsss2*Qsss2*Osss22
 ...
 totalAmplitude = 3D0*EL2*m2**2*CA2/(4D0*MW2*SW2*SB2)*(EL2/(2D0*(4D0*PI)**3*m1))*&
     &(16D0*PI*m1**3)/DSQRT(m1**4 + m2**4 + m3**4 - 2D0*m1**2*m2**2 - 2D0*m1**2*m3**2 - 2D0*m2**2*m3**2 )*&
     &( 2D0*( OLLFull ) + 2D0*( OLRFull ) )

 h0toTTBarReal = totalAmplitude
end function h0toTTBarReal
