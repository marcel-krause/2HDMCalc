double precision function h0toWmWpReal()
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 double precision :: Qssv0, Qssv1, Qssv2, Qsvv1, Qsvv2, QsvvFull
...
QsvvFull = Qsvv1*Qsvv1*Osvv11 + Qsvv1*Qsvv2*Osvv12 + Qsvv2*Qsvv2*Osvv22
 ...
 totalAmplitude = 1D0*(EL2*MW2*SBA2/SW2)*(EL2/(2D0*(4D0*PI)**3*m1*m2**2*m3**2))*&
     &(16D0*PI*m1**3)/DSQRT(m1**4 + m2**4 + m3**4 - 2D0*m1**2*m2**2 - 2D0*m1**2*m3**2 - 2D0*m2**2*m3**2 )*&
     &( QsvvFull )

 h0toWmWpReal = totalAmplitude
end function h0toWmWpReal
